                   ...
                   # print(val)
                    if val[0]==b:
                        ...
                        if val[1].state=='normal':
                            ...
                            sequencepool[trackselected-1]=sorted(sequencepool[trackselected-1], key=operator.itemgetter(0))
                            sequencepool2[trackselected-1].append([(bx-1)*zoom+rangeX+1,by+rangeY-1,"note on"])
                            sequencepool2[trackselected-1].append([(bx-1)*zoom+rangeX+1+zoom,by+rangeY-1,"note off"])
                            # sequencepool2[trackselected-1]=sorted(sequencepool[trackselected-1], key=operator.itemgetter(0))
                            print(sequencepool)
                            pprint(sequencepool2)
                ...
                for val in self.ids.items():
                   # print(val)
                    if val[0]==b:
                        ...
                        #sequencepool[trackselected-1]=sorted(sequencepool[trackselected-1], key=operator.itemgetter(0))
                        #print(sequencepool)
                        sequencepool2[trackselected-1].append([(bx-1)*zoom+rangeX+1,by+rangeY-1,"note_off"])
                        sequencepool2[trackselected-1]=sorted(sequencepool[trackselected-1], key=operator.itemgetter(0))
                        pprint(sequencepool2)
                        print("released")
            ...
            if loopstate[n] <= len(sequencepool2[n])-1:
                ...
                while sequencepool2[n][loopstate[n]][0]==count%loopsize[n]:
                    ...
                    if sequencepool2[n][loopstate[n]][2]==1:
                        print ("send" , sequencepool2[n][loopstate[n]][1] ,"channel" , n+1)
                        if n < 15:
                            channel=n
                        else:
                            channel=1
...
sequencepool2=[[[12,41,1],[17,42,1],[25,42,1],[25,40,1],[26,45,1],[29,42,1],[29,45,1],[70,48,1]],[[17,42,1]],[[12,41,1],[17,42,1],[70,42,1]],[[17,42,1]],[[17,42,1]],[[5,39,1],[13,40,1]],[],[],[],[[17,42,1]],
[],[],[],[],[],[],[],[],[],[],
[],[],[],[],[],[],[],[],[],[],
[],[],[],[],[],[],[],[],[],[],
[],[],[],[],[],[],[],[],[],[],
